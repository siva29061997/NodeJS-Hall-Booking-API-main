...
app.post("/createRoom", function (req, res) {
    ...
    room.roomNo = roomNo;
    room.bookings = [];
    if(req.body.noSeats){room.noSeats = req.body.noSeats} else{res.status(400).json({ output: 'Please type noSeats:....'})};
    if(req.body.amenities){room.amenities = req.body.amenities} else{res.status(400).json({ output: 'Please type amenities:....'})};
    if(req.body.price){room.price = req.body.price} else{res.status(400).json({ output: 'Please type price:...'})};
    rooms.push(room);
    roomNo++;
...
app.post("/createBooking", function (req, res) {
    let booking = {};
    booking.id = uniqid();
    if(req.body.custName){booking.custName = req.body.custName} else{res.status(400).json({ output: 'Please type custName:....'})};
    if(req.body.date){
    ...
    if(req.body.startTime){
        if (time_regex.test(req.body.startTime)) {
            ...
            res.status(400).json({ output: 'Please specify time in hh:min(24-hr format) where minutes should be 00 only ex(17:00:00)'})
        }
    } else{
    ...
    if(req.body.endTime){
        if (time_regex.test(req.body.endTime)) {
            ...
            res.status(400).json({ output: 'Please specify time in hh:min(24-hr format) where minutes should be 00 only ex(20:00:00)'})
        }
    } else{
    ...
    if(availableRooms.length == 0){res.status(400).json({ output: 'first create rooms (No Available Rooms on Selected Date and Time)'})}
   else{
